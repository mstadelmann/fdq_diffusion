...
class H5Dataset(data.Dataset):
    ...
    def __init__(
        self, file_paths, data_transform=None, hdf_ds_key_request=None, data_is_3d=False
    ):
        """
        file_paths (list): List of file paths.
        data_transform : already initialised data transformer.
        ...
        for file_idx, fp in enumerate(self.dataset_file_paths):
            ...
            has_key_requests = (
                isinstance(hdf_ds_key_request, list) and len(hdf_ds_key_request) > 0
            )

            datasets_are_sample_pairs = has_key_requests and all(
                elt in root_keys for elt in hdf_ds_key_request
            )
            ...
            groups_are_sample_pairs = has_key_requests and all(
                elt in group_names for elt in hdf_ds_key_request
            )
    ...
    def _get_transform(self, field_key=None):
        if field_key is None:
            return self.data_transform
        elif self.data_transform is None:
            return None
        else:
            return self.data_transform.get(field_key, None)
    ...
    def _get_sample(self, index, file_idx):
        ...
        if self.av_group_names == ["ROOTGROUP"]:
            ...
            if self.data_transform is None:
                ...
                return tuple(self.data_transform(ds[sample_idx, :].unsqueeze(0)))
    ...
    def _get_group_sample(self, index, file_idx):
        ...
        for group_idx, req_group in enumerate(self.hdf_ds_key_request):
            ...
            transform = self._get_transform(req_group)
            if transform is None:
                all_data.append(ds)
            else:
                all_data.append(transform(ds))
